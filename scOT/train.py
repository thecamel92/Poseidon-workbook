...
def setup(params, model_map=True):
    ...
    try:
        ...
        #macOS has no cpu_affinity; fall back to the core count
        CPU_CORES = os.cpu_count() or 1
    CPU_CORES = min(CPU_CORES, 16)
    if sys.platform == "darwin":
        #macOS spawns dataloader workers instead of forking them, and the
        #datasets hold an open h5py reader, which cannot be pickled
        CPU_CORES = 0
